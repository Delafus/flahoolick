...
export function Header() {
  ...
  /* Scrolled state for subtle border */
  useEffect(() => {
  ...
  return (
    <header
      ...
      style={{
        backgroundColor: active || mobileOpen ? bg : bg,
        borderBottom: scrolled && !active ? `1px solid ${divColor}` : '1px solid transparent',
      }}
    >
      {/* ── NAV BAR ── */}
      <div
        className="flex items-center justify-between page-px"
        style={{ height: '64px', color: text }}
      >
        {/* Left nav */}
        ...
        >
          <Shield color={text} size={22} />
          <span className="label" style={{ color: text, letterSpacing: '0.18em' }}>FLAHOOLICK</span>
        </Link>
      ...
      {mobileOpen && (
        <div
          className="md:hidden page-px py-8 flex flex-col gap-8"
...
/* ──────────────────────────────────────────────
   MEGA MENU CONTENT — same layout as NOBL
   Left col: tagline + CTA | Divider | Right: 3 items
────────────────────────────────────────────── */
function MegaMenu({ menu, textColor, divColor, onClose }: {
  menu: typeof CAP_MENU
  textColor: string
  divColor: string
  onClose: () => void
}) {
  return (
    <div className="page-px py-12 max-container w-full mx-auto grid grid-cols-12 gap-8">
      ...
      <div className="col-span-4 flex flex-col justify-between gap-12">
        <p className="text-2xl font-light leading-snug" style={{ color: textColor }}>
          {menu.tagline}
        </p>
        <Link
          href={menu.ctaHref}
      ...
      <div className="col-span-7 grid grid-cols-3 gap-8">
        {menu.items.map(item => (
          <Link key={item.href} href={item.href} onClick={onClose} className="group flex flex-col gap-3">
            ...
            <p className="text-sm leading-relaxed opacity-60" style={{ color: textColor }}>
              {item.desc}
            </p>
          </Link>
        ))}
...
function Shield({ color, size = 24 }: { color: string; size?: number }) {
  return (
    <svg width={size} height={Math.round(size * 1.15)} viewBox="0 0 28 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M14 0L0 5.33333V16C0 24 6 30.6667 14 32C22 30.6667 28 24 28 16V5.33333L14 0Z" fill={color} />
    </svg>
  )
}
